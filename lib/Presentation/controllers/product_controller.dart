...
class ProductController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    loadProductsForCurrentOptician();
    loadProducts();
    loadOpticiens();
  }
  ...
  Future<void> loadAllProductsForOptician() async {
    try {
      ...
      // Debug: Print boutique count
      print('Boutiques found: ${boutiqueController.opticiensList.length}');
      ...
      if (boutiqueIds.isNotEmpty) {
        ...
        for (final boutiqueId in boutiqueIds) {
          try {
            ...
            // Debug: Print products count per boutique
            print('Products for boutique $boutiqueId: ${products.length}');
            allProducts.addAll(products);
          } catch (e) {
            print('Error loading products for boutique $boutiqueId: $e');
          }
        }
        ...
        _products.clear();
        _allProducts.clear();
        print('No boutiques found for this optician');
      }
    } catch (e) {
      _error.value = 'Error: ${e.toString()}';
      print('Error in loadAllProductsForOptician: $e');
    } finally {
      _isLoading.value = false;
    }
  }
  ...
  Future<void> loadProductsForCurrentOptician() async {
    try {
      ...
      if (!opticianController.isLoggedIn.value) {
        throw Exception('Opticien non connecté');
      }
      ...
      _error.value = 'Erreur: ${e.toString()}';
      rethrow;
    } finally {
      _isLoading.value = false;
    }
...
Future<bool> updateProduct(String id, Product product) async {
  try {
    ...
    // Mise à jour dans la liste des produits
    final index = _products.indexWhere((p) => p.id == id);
    if (index != -1) {
      _products[index] = updatedProduct;
      _products.refresh(); // Cette ligne est cruciale
    }

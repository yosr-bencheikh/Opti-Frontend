...
class MonthlyOpticianSalesChart extends StatefulWidget {
  ...
  @override
  State<MonthlyOpticianSalesChart> createState() =>
      _MonthlyOpticianSalesChartState();
}
          ...
          child: Padding(
            ...
            child: Obx(() {
              ...
              final opticianOrders = orderController.allOrders.where((order) {
                return order.items
                    .any((item) => boutiqueIds.contains(item.boutiqueId));
              }).toList();
  ...
  Map<DateTime, double> _processDailySalesData(
    ...
    for (final order in filteredOrders) {
      ...
      if (orderDate.isAfter(month.subtract(const Duration(seconds: 1))) &&
        ...
        double orderTotal =
            order.total; // Use order.total which includes all items
        dailySales.update(
          dayKey,
      ...
      lineBarsData: [
        LineChartBarData(
          ...
          preventCurveOverShooting:
              true, // Empêche la courbe de dépasser les valeurs réelles
          curveSmoothness: 0.4,
